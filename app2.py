          ...
          if st.button("Generate Insights",disabled=st.session_state.disabled):
                if st.session_state.llm == "Closed-Source":
                    ...
                    prompt = f" You are professional Fraud Analyst. Find answer to the questions as truthfully and in as detailed as possible as per given context only,\n\n\
                        1. The transaction/disputed amount above 5,000 USD value threshold. \n\n\
                        2. There is an indication of suspicion with involvement of multiple individuals whose details mismatch with customer details. (Customer details can be identified from cardholder's information) \n\n\
                        ...
                        If transaction/disputed amount is below 5000 USD threshold and no suspicious activity is detected based on above mentioned points, write your response as - There is no indication of suspicious activity.\n\n\
                        Context: {contexts}\n\
...
with col_d2:
    ...
    with col5_up:   
        ...
        if st.session_state.llm == "Closed-Source":
            ...
            If transaction/disputed amount is below 5000 USD threshold and no suspicious activity is detected based on above mentioned points, write your response as - There is no indication of suspicious activity.Therefore,no requirement to file SAR with FinCEN.\n\n\
                    Question: {query}\n\
                    Context: {context_1}\n\                      
