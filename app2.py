...
with col_d2:
    ...
    with col5_up:   
        ...
        if st.session_state['tmp_table_gpt'] is not None:
            ...
            st.markdown(f'''<em style="font-size: 20px;">{response_sara_gpt}</em>''',unsafe_allow_html=True)
            ...
            # st.markdown(response1)
            st.markdown(f'''<em style="font-size: 20px;">{response_sara_llama}</em>''',unsafe_allow_html=True)
